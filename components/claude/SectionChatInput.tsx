  ...
  return (
    <div className="mt-3 space-y-2">
      ...
      {isLoading && activityLog.length > 0 && (
        <div
          ref={activityRef}
          className="max-h-[120px] overflow-y-auto rounded-lg border-2 border-purple-600 bg-purple-100 dark:bg-purple-950/50 p-3 space-y-1.5"
        >
          ...
          {activityLog.slice(-5).map((entry, i) => (
            <div key={i} className="flex items-start gap-2 text-xs">
              {entry.type === "thinking" && (
                <>
                  <Brain className="h-3 w-3 mt-0.5 text-purple-600 dark:text-purple-400 flex-shrink-0" />
                  <span className="text-purple-700 dark:text-purple-200 italic">{truncate(entry.content, 200)}</span>
                </>
              )}
              {entry.type === "tool_use" && (
                <>
                  <Wrench className="h-3 w-3 mt-0.5 text-blue-600 dark:text-blue-400 flex-shrink-0" />
                  <span className="text-blue-700 dark:text-blue-200 font-medium">{entry.content}</span>
                </>
              )}
