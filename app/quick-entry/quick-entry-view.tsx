...
export interface QuickEntryViewProps {
  /** Extra nodes rendered at the end of the badges bar (e.g. cloud assignee badge). */
  extraBadges?: ReactNode;
  /** Extra fields merged into the POST /api/cards body on submit and ideate. */
  extraPayload?: Record<string, unknown>;
  /** Fired when the selected project changes; lets wrappers mirror it. */
  onSelectedProjectChange?: (project: Project | null) => void;
  /** Fired when the Electron reset-quick-entry event handler runs. */
  onReset?: () => void;
}
...
export function QuickEntryView({
  ...
  const handleIdeate = useCallback(async () => {
    ...
    closeWindow();
  }, [canIdeate, title, complexity, priority, aiPlatform, selectedProject, extraPayload, closeWindow, rememberSelection]);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    closeWindow();
  }, [title, status, complexity, priority, aiPlatform, selectedProject, extraPayload, closeWindow, rememberSelection]);
